...
class File extends INode {
    ...
    read(opts, done) {
        if (!done || !_.isFunction(done)) {
            return new Promise((resolve, reject) => {
                return this.read(opts, (error, results) => {
    ...
    write(content, opts, done) {
        if (!done || !_.isFunction(done)) {
            return new Promise((resolve, reject) => {
                return this.write(content, opts, (error, results) => {
                    if (error) return reject(error)
                    return resolve(results)
                })
            })
    ...
    copy(destPath, opts, done) {
        if (!done || !_.isFunction(done)) {
            return new Promise((resolve, reject) => {
                return this.copy(destPath, opts, (error, results) => {
                    if (error) return reject(error)
                    return resolve(results)
                })
            })
        ...
        return this.read({ encoding }, (error, content) => {
            if (error) return done(error, null)
    ...
    makeLink(destPath, done) {
        if (!done || !_.isFunction(done)) {
            return new Promise((resolve, reject) => {
                return this.makeLink(destPath, (error, results) => {
                    if (error) return reject(error)
                    return resolve(results)
                })
            })
